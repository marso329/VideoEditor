...
Video::Video(QObject* parent) :
		QObject(parent), currentFrame(NULL), open_(false), pFormatCtx(NULL), pVideoCodecCtx(
		NULL), pVideoCodec(NULL), pAudioCodecCtx(NULL), pAudioCodec(
		NULL), codecName(""), videoStreamIndex(-1), audioStreamIndex(-1), videoFramePerSecond(
				0), durationUs(0), durationS(0), nb_frames(0), videoBaseTime(
				0.0), audioBaseTime(0.0), pImgConvertCtx(
		NULL), width(0), height(0),position(0.0f) {

}

Video::Video(std::string filename, QObject* parent) :
		QObject(parent), currentFrame(NULL), open_(false), pFormatCtx(NULL), pVideoCodecCtx(
		NULL), pVideoCodec(NULL), pAudioCodecCtx(NULL), pAudioCodec(
		NULL), codecName(""), videoStreamIndex(-1), audioStreamIndex(-1), videoFramePerSecond(
				0), durationUs(0), durationS(0), nb_frames(0), videoBaseTime(
	...
	if (pFormatCtx) {
		...
		if (!res) {
			...
			pImgConvertCtx = sws_getContext(pVideoCodecCtx->width,
					pVideoCodecCtx->height, pVideoCodecCtx->pix_fmt,
					pVideoCodecCtx->width, pVideoCodecCtx->height,
					AV_PIX_FMT_RGB555,
					SWS_BICUBIC, NULL, NULL, NULL);
		}
		if (videoStreamIndex != -1) {
			videoFramePerSecond = av_q2d(
					pFormatCtx->streams[videoStreamIndex]->r_frame_rate);
			// Need for convert time to ffmpeg time.
			videoBaseTime = av_q2d(
	...
	_filename = filename;
	_filenameWithoutPath = _filename.substr(_filename.find_last_of("\\/") + 1,
			_filename.size() - _filename.find_last_of("\\/") - 1);
	insertFrames();
}
...
void Video::insertFrames() {
	...
	if (videoStreamIndex != -1) {
		...
		if (open_) {
			// Read packet.
			while (av_read_frame(pFormatCtx, &temp_package) >= 0) {
				if (temp_package.stream_index == videoStreamIndex) {
					Frame* frame = new Frame(this, pFormatCtx, temp_package.dts,videoStreamIndex);
	...
	if (pVideoCodecCtx) {
		if (avpkt && pOutFrame) {
			...
			int videoFrameBytes = avcodec_decode_video2(pVideoCodecCtx,
					pOutFrame, &got_picture_ptr, avpkt);
...
std::string titles_init[6] = { "Width", "Height", "FPS", "Duration_sec",
		"CodecName", "NBFrames" };
